...
@Service
public class CloneableCommands implements CommandMarker { // All command types must implement the CommandMarker interface
    
    /**
     * Get a reference to the CloneableOperations from the underlying OSGi container
     */
    @Reference private CloneableOperations operations;
     ...
     */
    @CliAvailabilityIndicator({ "cloneable setup", "cloneable add", "cloneable all" })
     ...
     * @param type 
     */
    @CliCommand(value = "cloneable add", help = "Some helpful description")
    public void add(@CliOption(key = "type", mandatory = true, help = "The java type to apply this annotation to") JavaType target,
          @CliOption(key = "fetch", mandatory = false, unspecifiedDefaultValue = "eager", help = "The fetching strategy for retrieving clone reference (eager or lazy)") FetchType fetch) {
       
        if (fetch == null){
           fetch = FetchType.eager;
        }
        operations.annotateType(target, fetch);
    }
     ...
     * 
     */
    @CliCommand(value = "cloneable all", help = "Some helpful description")
    public void all() {
     ...
     * 
     */
    @CliCommand(value = "cloneable setup", help = "Setup Cloneable addon")
    public void setup() {
        operations.setup();
    }
}