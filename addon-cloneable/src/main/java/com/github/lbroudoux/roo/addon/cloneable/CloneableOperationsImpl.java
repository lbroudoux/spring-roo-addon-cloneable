...
@Service
public class CloneableOperationsImpl implements CloneableOperations {
    ...
    /** {@inheritDoc} */
    public void annotateType(JavaType javaType, FetchType fetch) {
        ...
        if (existing != null && MemberFindingUtils.getAnnotationOfType(existing.getAnnotations(), new JavaType(RooCloneable.class.getName())) == null) {
            ...
            JavaType rooRooCloneable = new JavaType(RooCloneable.class.getName());

            // Create Annotation metadata
            AnnotationMetadataBuilder annotationBuilder = new AnnotationMetadataBuilder(rooRooCloneable);
            
            annotationBuilder.addEnumAttribute("fetch", new JavaType(FetchType.class.getName()), fetch.toString());
    ...
    public void annotateAll() {
        ...
        for (JavaType type: typeLocationService.findTypesWithAnnotation(new JavaType("org.springframework.roo.addon.javabean.RooJavaBean"))) {
            annotateType(type, FetchType.eager);
        }
    }
    ...
    public void setup() {
        ...
        projectOperations.addRepository("", new Repository("Cloneable Roo add-on repository", "Cloneable Roo add-on repository", "https://raw.github.com/lbroudoux/spring-roo-addon-cloneable/master/repo"));
        
        List<Dependency> dependencies = new ArrayList<Dependency>();
        
        // Install the dependency on the add-on jar (
        dependencies.add(new Dependency("com.github.lbroudoux.roo.addon", "com.github.lbroudoux.roo.addon.cloneable", "0.1.0.BUILD", DependencyType.JAR, DependencyScope.PROVIDED));
